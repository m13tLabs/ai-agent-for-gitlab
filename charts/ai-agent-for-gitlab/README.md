...
| agentImage | object | `{"repository":"m13t/ai-agent-for-gitlab-agent","tag":"0.3.1"}` | Image the CI job uses to run the agent, forwarded to pipelines as AI_AGENT_IMAGE=<repository>:<tag>. Pipeline variables override the default in .gitlab-ci.yml, so every project uses this image. Set repository to "" to keep each project's own AI_AGENT_IMAGE. |
| agentImage.tag | string | `"0.3.1"` | Set to the release version by each release (scripts/pin-release-version.sh). "" falls back to .Chart.AppVersion. |
...
| image.tag | string | `"0.3.1"` | Set to the release version by each release (scripts/pin-release-version.sh). "" falls back to .Chart.AppVersion. |
...
| secrets.webhookSecret | string | `""` | Secret token configured on the GitLab webhook. Generated (and kept across upgrades) when empty; read it from the Secret shown in the install notes. Set it explicitly with `helm template`/GitOps tools, where `lookup` can't see the cluster and a generated value would change on every render. |
